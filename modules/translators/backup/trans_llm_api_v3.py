...
# -------------------------------------------------------------------------
try:
    from ui.module_manager import TranslateThread
...
# Monkey patch ImgTranlsatePipeline.save() to use global lock
if ImgTranlsatePipeline and not hasattr(ImgTranlsatePipeline, '_original_save'):
    ImgTranlsatePipeline._original_save = ImgTranlsatePipeline.save
    
    def save_with_lock(self, keep_exist_as_backup=False):
        with _GLOBAL_SAVE_LOCK:
            return self._original_save(keep_exist_as_backup)
    
    ImgTranlsatePipeline.save = save_with_lock
    if LOGGER:
        LOGGER.info("Monkey patch applied: Global save lock protection")
...
def _run_translate_pipeline_patched(self):
    ...
    if LOGGER:
        LOGGER.info(f"🚀 V3 Parallel Processing: {self.translator.concurrent_images} images/batch.")
    ...
    last_save_time = 0.0
    completed_pages = []  # Track pages to save in batch
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        while not self.pipeline_finished():
            ...
            # Wait for initial buffer before starting (only once)
            if not has_started:
                queue_len = len(self.pipeline_pagekey_queue)
                if queue_len >= initial_buffer:
                    has_started = True
                    if LOGGER:
                        LOGGER.info(f"Initial buffer reached ({queue_len} pages). Starting parallel translation with {max_workers} workers.")
                else:
                    time.sleep(0.1)  # Reduced from 0.2 to 0.1
                    continue

            # Aggressive work submission: fill up to max_workers immediately
            submitted_count = 0
            ...
            if submitted_count > 0 and LOGGER:
                LOGGER.debug(f"Submitted {submitted_count} new translation tasks. Active: {len(futures)}/{max_workers}")
            
            if not futures:
                time.sleep(0.05)  # Reduced from 0.1 to 0.05
                continue

            # Wait with timeout to check queue more frequently
            done, _ = concurrent.futures.wait(
                futures.keys(), 
                timeout=0.1,  # Check every 0.1s for new work
            ...
            for future in done:
                ...
                try:
                    ...
                    trans_success = False
                    error_msg = f"번역 실패 (Page: {page_key}): {type(e).__name__} - {str(e)}"
                    if LOGGER:
                    ...
                    if shared and getattr(shared, 'create_errdialog_in_mainthread', None):
                        try:
                            shared.create_errdialog_in_mainthread(error_msg, "Translation Error", "TranslationFailed")
                        except:
                            pass
                ...
                if trans_success:
                    ...
                    with _GLOBAL_SAVE_LOCK:
                        ...
                        is_queue_empty = len(self.pipeline_pagekey_queue) == 0 and len(futures) == 0
                        
                        # Save project at interval or when queue is empty
                        if is_queue_empty or (current_time - last_save_time >= save_interval):
                            if completed_pages:
                                self.imgtrans_proj.save()
                                completed_pages.clear()
                                last_save_time = current_time
                                if LOGGER:
                                    LOGGER.debug(f"Saved project with {self.finished_counter} completed translations")
                else:
                    # Failed translation - skip from progress
                    if LOGGER:
                        LOGGER.error(f"Page {page_key} translation failed - skipping from progress")
        
        # Translation pipeline finished - Notification only
        if LOGGER:
            LOGGER.info("Translation pipeline completed! (Save handled by OCR thread)")
