...
def _apply_chunked_processing_patch():
    ...
    try:
                        ...
                        for page_idx in range(proj.num_pages):
                            ...
                            try:
                                ...
                                while time.time() - start_time < timeout:
                                    # Process events to allow UI items to initialize
                                    app.processEvents()
        ...
        def _imgtrans_pipeline_async(self):
            # Check if current OCR module requires chunking (V2, V3 or PaddleVL)
            ...
            # -----------------------------------------------------------------
            # FINAL SAVE STEP (Wait for Parallel Translation to finish first)
            # -----------------------------------------------------------------
            if self.parallel_trans and cfg_module.enable_translate:
                if LOGGER:
                    LOGGER.info(f"Waiting for parallel translation to complete... (Target: {self.num_pages} pages)")
                
                # Wait until finished_counter matches num_pages
                # This ensures we don't start saving until ALL pages are actually done
                while self.translate_thread.finished_counter < self.num_pages:
                    if self.stop_requested:
                        break
                    time.sleep(0.5)
                
                if LOGGER:
                    LOGGER.info(f"Translation confirmed complete ({self.translate_thread.finished_counter}/{self.num_pages}).")
            
            # Now run the Smart Wait Save (This is the one that was working!)
            if LOGGER:
                LOGGER.info("Pipeline tasks completed. Initiating Verified Save Sequence...")
