...
@register_OCR("llm_ocr_v2")
class LLM_OCR_V2(OCRBase):
    ...
    params = {
        ...
        "prompt": {
            ...
            "description": "OCR 작업을 위한 프롬프트입니다. {language} 플레이스홀더를 사용하세요.",
        },
        "max_response_tokens": {
            "value": 4096,
        ...
        "description": "비전 LLM을 사용한 OCR with async batch processing.",
    }
    ...
    def __init__(self, **params) -> None:
        ...
        if not LLM_OCR_V2._patch_applied:
            LLM_OCR_V2._patch_applied = True
    ...
    def _ocr_with_retry(self, img_base64: str, prompt_override: str = None, model_override: str = None, retry_override: int = None) -> str:
        ...
        if response.choices and response.choices[0].message.content:
            full_text = response.choices[0].message.content.replace("\n", " ").strip()
...
def _apply_chunked_processing_patch():
    ...
    try:
        ...
        if LOGGER:
            LOGGER.info("LLM OCR V2: Attempting to apply async pipeline patch...")
        ...
        def _imgtrans_pipeline_async(self):
            ...
            if LOGGER:
                LOGGER.info(f"LLM OCR V2: is_target_ocr={is_target_ocr}, OCR class={self.ocr.__class__.__name__}, use_page_batching={getattr(self.ocr, 'use_page_batching', 'N/A')}")
            
            if not is_target_ocr:
                if LOGGER:
                    LOGGER.info("LLM OCR V2: Using original pipeline (not target OCR)")
            ...
            # --- ASYNC LOGIC ---
            # Get settings from OCR module
            ocr_workers = getattr(self.ocr, 'ocr_parallel_workers', 1)
            ...
            # max_workers=N for parallel OCR (if VRAM allows)
            if LOGGER:
                LOGGER.info(f"🚀 Async Pipeline: OCR workers={ocr_workers}, Initial buffer={initial_buffer} pages")
        ...
        if LOGGER:
            LOGGER.error(f"LLM OCR V2: Failed to apply async pipeline patch (ImportError): {e}")
    except Exception as e:
        if LOGGER:
            LOGGER.error(f"LLM OCR V2: Failed to apply async pipeline patch: {e}")